...
export default function RotatingEarth({ width = 1500, height = 1000, className = "" }: RotatingEarthProps) {
  const canvasRef = useRef<HTMLCanvasElement>(null)
  const [isLoading, setIsLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)

  // Function to move/center the globe on specific coordinates
  const moveToLocation = (latitude: number, longitude: number, zoom: number = 14) => {
    console.log('[GLOBE] moveToLocation called:', { latitude, longitude, zoom });
    // For now, just log the coordinates - in a real implementation,
    // you might want to animate the camera or highlight specific areas
  };
  ...
  useEffect(() => {
    ...
    const containerWidth = Math.min(width, window.innerWidth * 0.7)
